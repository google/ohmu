...
static void printCompare(Event* event, const char* name) {
  unsigned copySet = event->copySet;
  unsigned reg0 = event[-2].liveRange.registerSet;
  unsigned reg1 = event[-1].kind == Event::DUPLICATE_LIVE_RANGE ? event[-2].liveRange.registerSet : event[-1].liveRange.registerSet;
  printf("CMP %s %s", getRegName(reg0), getRegName(reg1));
  while (copySet) {
    unsigned copyReg = copySet & -copySet;
    printf("\n    SET%s %s", name, getRegName(copyReg));
    copySet &= ~copyReg;
  }
}

static void printBranch(Event* event) {
  printf("TEST %s %s\n    JZ %d", getRegName(event[-1].liveRange.registerSet), getRegName(event[-1].liveRange.registerSet), event->branchTarget);
}
...
void Event::printASM(Event* events) {
  if (kind == Event::INT_LITERAL) {
    ...
    while (copySet) {
      unsigned copyReg = copySet & -copySet;
      printf("\n    MOV %s %d", getRegName(copyReg), intLiteral);
      copySet &= ~copyReg;
    }
    return;
  }
  ...
  if (kind != Event::INSTRUCTION)
    return;
  switch (opcode) {
  case Event::JMP: if (events + branchTarget != this + 1) printf("JMP %d", branchTarget); break;
  case Event::BRANCH: printBranch(this); break;
  case Event::ADD: printCommutable(this, "ADD"); break;
  case Event::MUL: printCommutable(this, "MUL"); break;
  case Event::CMP_EQ: printCompare(this, "E"); break;
  case Event::CMP_LT: printCompare(this, "LT"); break;
  case Event::CMP_LE: printCompare(this, "LE"); break;
  };
}
...
RegisterAllocator::RegisterAllocator(SCFG* cfg) {
  ...
  for (auto block : *cfg) {
    if (block->terminator()) {
      ...
      case COP_Goto:
        events[block->VX64BlockEnd].branchTarget = cast<Goto>(block->terminator())->targetBlock()->VX64BlockStart;
        break;
      case COP_Branch:
        events[block->VX64BlockEnd].branchTarget = cast<Branch>(block->terminator())->elseBlock()->VX64BlockStart;
        break;
      }
    }
  }
...
unsigned RegisterAllocator::determineClobberSet(Event* event, unsigned& sourceCopySet) {
  ...
  for (Event* i = event - 1, *e = event + event->liveRange.offsetToOrigin; i != e; --i) {
    ...
    if (i->kind == Event::LIVE_RANGE) {
      ...
      clobberSet |= i->liveRange.registerSet | i->copySet | i->liveRange.clobberedSet;
    }
  }
  return clobberSet;
}
